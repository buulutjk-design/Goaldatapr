...
# ── LİGLER — ÖNCELİK SIRALI ─────────────────────────────────

# Önce bol gollü ligler
...
xg_vals = []
for fid in fixture_ids[:4]:  # API tasarrufu için max 4
...
def get_h2h(id1, id2):
r = safe_request(f”{API_URL}/fixtures/headtohead?h2h={id1}-{id2}&last=8”)
if not r:
return []
...
# xG düzeltmesi
if home_xg > 0: lh = lh * 0.75 + home_xg * 0.25
if away_xg > 0: la = la * 0.75 + away_xg * 0.25
...
# Güvenilirlik
warns = []
if len(home_gen) < 4: warns.append("low_home")
if len(away_gen) < 4: warns.append("low_away")
if not h2h:           warns.append("no_h2h")
...
if prob >= 80: return “YÜKSEK GÜVEN - GÜVENLİ 🍀”
if prob >= 72: return “İYİ GÜVEN - GÜVENLİ 🍀”
return “ORTA GÜVEN ⚠️”
...
```
league_line = f"Lig: {league}\n" if league else ""
ko_line     = f"⏰ {kickoff}\n"  if kickoff else ""

# 1.5 karar
if o15 >= CONFIDENCE_THRESHOLD:
    v15 = f"✅ 1.5 ÜST  {o15}%  [ {confidence_label(o15)} ]"
else:
    v15 = f"⬇️ 1.5 ALT  {u15}%  [ DÜŞÜK GOL BEKLENTİSİ ]"

# 2.5 karar
if o25 >= CONFIDENCE_THRESHOLD:
    v25 = f"✅ 2.5 ÜST  {o25}%  [ {confidence_label(o25)} ]"
else:
    v25 = f"⬇️ 2.5 ALT  {u25}%  [ DÜŞÜK GOL BEKLENTİSİ ]"

# Son 6 maç
s6 = (
    f"Son 6: 🏳 {result['ho15']}/6  🚩 {result['ao15']}/6  (1.5)\n"
    f"Son 6: 🏳 {result['ho25']}/6  🚩 {result['ao25']}/6  (2.5)\n"
)

h2h_line = f"H2H 2025+: {result['h2h_count']} maç\n" if result["h2h_count"] > 0 else "H2H 2025+: Veri yok\n"
...
return (
    ...
    f"🚩 Dep: {result['a']}\n"
    f"{ko_line}\n"
    f"⚡ xG: 🏳 λ{result['lh']}  🚩 λ{result['la']}  Σ{result['lam']}\n\n"
    "━━━━━━━━━━━━━━━━━━━━━━\n\n"
    f"{v15}\n\n"
    f"{v25}\n\n"
    "━━━━━━━━━━━━━━━━━━━━━━\n"
...
```
# Önce bol gollü ligler, sonra diğerleri
league_order = list(PRIORITY_LEAGUES.keys()) + [
    lid for lid in ALL_LEAGUES if lid not in PRIORITY_LEAGUES
]
...
“⚽️ Otomatik /banko Maç Analizi\n\n”
“🔥 %72+ güven duvarı\n”
“🌍 Tüm dünya ligleri\n”
“🥇 Bol gollü ligler öncelikli\n\n”
“👉 /banko — Analiz başlat\n”
“👉 /dur   — Botu durdur”
)
...
if not bot_active:
    await update.message.reply_text(
        "🔴 Bot durduruldu.\n👉 /banko ile yeniden başlat."
    )
    bot_active = True
    return

wait = await update.message.reply_text(
    "🛜 Bültendeki maçlar analiz ediliyor…\n"
    "Bol gollü liglerden başlıyorum."
)
...
try:
    ...
    if not fix or not result:
        await wait.edit_text(
            "❌ Bugün %72+ güvenli maç bulunamadı.\n\n"
...
bot_active = False
await update.message.reply_text(
“🔴 Bot durduruldu. API çekimi durduruldu.\n”
“💡 Tasarruf modu aktif.\n\n”
“👉 /banko ile yeniden başlat.”
)
